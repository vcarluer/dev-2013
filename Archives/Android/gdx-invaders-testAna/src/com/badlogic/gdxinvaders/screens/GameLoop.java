...
public class GameLoop implements Screen, SimulationListener {
	...
	@Override public void update (Application app) {
		...
		this.renderer.acceX = - input.getAccelerometerY();
		this.renderer.acceZ = input.getAccelerometerX();
		...
		if (input.isKeyPressed(Keys.KEYCODE_DPAD_LEFT)) this.renderer.acceX -= 0.1f;
		if (input.isKeyPressed(Keys.KEYCODE_DPAD_RIGHT)) this.renderer.acceX += 0.1f;
		if (input.isKeyPressed(Keys.KEYCODE_DPAD_UP)) this.renderer.acceZ += 0.1f;
		if (input.isKeyPressed(Keys.KEYCODE_DPAD_DOWN)) this.renderer.acceZ -= 0.1f;
